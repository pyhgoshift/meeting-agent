...
function parseResponse(raw: string, _retryFn: () => Promise<MeetingAnalysisFull>): MeetingAnalysisFull | null {
...
export async function analyzeStructured(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    let parsed = parseResponse(response.content, async () => { throw new Error('unused'); });

    if (!parsed) {
      response = await routedCall(mode, 'json_structure', systemPrompt, userMsg, transcript);
      parsed = parseResponse(response.content, async () => { throw new Error('unused'); });
      if (!parsed) {
        response = await routedCall(mode, 'json_structure', systemPrompt, userMsg, transcript);
        parsed = parseResponse(response.content, async () => { throw new Error('unused'); });
  ...
  return { analysis: mergeResults(results), modelUsed: lastModel, costUsd: totalCost, chunks: chunks.length };
}
