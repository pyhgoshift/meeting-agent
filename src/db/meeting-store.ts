...
export function createMeeting(id: string, fileName: string, mode: string): void {
  const db = getDb();
  db.prepare(`
    INSERT OR IGNORE INTO meetings (id, title, file_name, mode, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(id, fileName, fileName, mode, nowIso(), nowIso());
}
...
export function saveTranscript(meetingId: string, text: string, durationSec: number): void {
  const id = `${meetingId}-tr`;
  getDb().prepare(`
    INSERT OR REPLACE INTO transcripts (id, meeting_id, text, duration_sec, created_at)
    VALUES (?, ?, ?, ?, ?)
  `).run(id, meetingId, text, durationSec, nowIso());
}

export function startSummary(meetingId: string): void {
  getDb().prepare(`
    INSERT OR REPLACE INTO summaries (meeting_id, status, created_at, updated_at)
    VALUES (?, 'processing', ?, ?)
  `).run(meetingId, nowIso(), nowIso());
}

export function completeSummary(meetingId: string, result: MeetingAnalysisFull, modelUsed: string, costUsd: number): void {
  getDb().prepare(`
    UPDATE summaries SET status='completed', result=?, model_used=?, cost_usd=?, updated_at=?
    WHERE meeting_id=?
  `).run(JSON.stringify(result), modelUsed, costUsd, nowIso(), meetingId);
}

export function failSummary(meetingId: string, error: string): void {
  getDb().prepare(`
    UPDATE summaries SET status='failed', error=?, updated_at=? WHERE meeting_id=?
  `).run(error, nowIso(), meetingId);
}

export function getMeetings(): unknown[] {
  return getDb().prepare(`
    SELECT m.id, m.title, m.mode, m.created_at,
           s.status, s.cost_usd
    FROM meetings m
